...
static void printConnectionData(
		struct gecko_msg_le_connection_opened_evt_t *device);
...
void appMain(gecko_configuration_t *pconfig)
{
	...
	while (1)
	{
		...
		case gecko_evt_le_connection_opened_id:
		{
			...
			// Time = Value x 1.25 ms, => 6x1.25 = 7.5ms
			gecko_cmd_le_connection_set_timing_parameters(_conn_handle, 0x0006,
					0x0006, 1, 600, 0, 0xffff);
			response = gecko_cmd_le_connection_set_preferred_phy(_conn_handle,
			PREFFERED_PHY, ACCEPTED_PHY)->result;
			if (!(response))
			{
		...
		case gecko_evt_gatt_server_user_write_request_id:
					...
					== gattdb_led0)
			{
				...
				gecko_cmd_gatt_server_send_user_write_response(_conn_handle,
						gattdb_led0, bg_err_success);
			}
			break;

			// Event handler for SPP data received from phone
		case gecko_evt_gatt_server_attribute_value_id:
		{
			struct gecko_msg_gatt_server_attribute_value_evt_t *pRxData =
		...
		case gecko_evt_gatt_server_user_read_request_id:
		{
			...
			if (pStatus->characteristic == gattdb_led0)
			{
				...
				gecko_cmd_gatt_server_send_user_read_response(_conn_handle,
						gattdb_led0, bg_err_success, 1, &status_byte);
			}
			else if (pStatus->characteristic == gattdb_adc_data)
			{
				...
				gecko_cmd_gatt_server_send_user_read_response(_conn_handle,
						gattdb_adc_data, bg_err_success, 4, &singleResult);
			}
			break;
		}
		...
		case gecko_evt_system_external_signal_id:
			...
			gecko_cmd_gatt_server_send_characteristic_notification(_conn_handle,
					gattdb_button_1, size_of_pb1_rise_count, &pb1_rise_count);
			break;
		...
		case gecko_evt_gatt_server_characteristic_status_id:
		{
				...
				if (pStatus->status_flags == gatt_server_client_config)
				{
					...
					if (pStatus->client_config_flags == gatt_notification)
					{
						...
						gecko_cmd_gatt_server_send_characteristic_notification(
								_conn_handle, gattdb_button_1,
								size_of_pb1_rise_count, &pb1_rise_count);
					}
					else
					{
			...
			else if (pStatus->characteristic == gattdb_uart2_data)
			{
				if (pStatus->status_flags == gatt_server_client_config)
				{
					// Characteristic client configuration (CCC) for spp_data has been changed
					if (pStatus->client_config_flags == gatt_notification
							|| pStatus->client_config_flags == gatt_indication)
					{
